...
- **場面選択** … 3つの場面ごとに「済んだ件数 / 全件数」と進捗バー（緑＝チェック、橙＝スキップ）
- **チェックリスト**
  - 行のどこを押してもチェック / 解除
  - 右端の「スキップ」を押すと、その項目は**やらなかった記録**として橙色になる（「戻す」で解除）
  - スキップもチェックも「済んだ件数」に数える。全部さばけば場面が完了扱いになる
  - 下の「✎ 編集」で、その場で項目の追加・編集・並べ替え・削除ができる
- **設定** … 場面名の変更、全項目のまとめ編集、バックアップ、スリープ設定
...
```jsonc
{
  ...
  "log": {
    // 業務日 → 場面ID → 項目ID → { at: 時刻(epoch ms), skip: スキップなら true }
    "2026-09-01": {
      "<sceneId>": {
...
version 1（項目IDの値が数値だけ）で書き出した古いバックアップも、そのまま読み込めます
（読み込み時に version 2 の形へ変換されます）。
...
Chromium（モバイル表示 384×832）で49項目を自動テスト済み：
場面選択・チェック/解除・行全体のタップ判定・スキップの3状態遷移と件数の数え方・時刻記録・
...
version 1 データの自動変換・Service Worker 制御下でのオフライン起動。
