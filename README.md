...
- **コンテキスト選択** … コンテキストごとに「済んだ件数 / 全件数」と進捗バー（緑＝チェック、橙＝スキップ）
- **チェックリスト**
  - 行のどこを押してもチェック / 解除
  - 右端の「スキップ」を押すと、その項目は**やらなかった記録**として橙色になる（「戻す」で解除）
  - スキップもチェックも「済んだ件数」に数える。全部さばけばコンテキストが完了扱いになる
  - 下の「✎ 編集」で、その場で項目の追加・編集・並べ替え・削除ができる
- **設定** … コンテキストの追加 / 削除 / 名前の変更、全項目のまとめ編集、バックアップ、スリープ設定
...
Chromium（モバイル表示 384×832）で60項目を自動テスト済み：
